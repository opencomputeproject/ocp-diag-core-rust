...
impl TestRun {
    ...
    pub async fn start(&self) -> Result<(), emitters::WriterError> {
        ...
        if self.metadata.is_some() {
            for m in self.metadata.as_ref().unwrap().into_iter() {
    ...
    /// let test_run = TestRun::new("diagnostic_name", "my_dut", "1.0");
    /// test_run.start().await?;
    /// test_run.end(TestStatus::Complete, TestResult::Pass)?;
    /// ```
    pub async fn end(
        &self,
    ...
    /// let test_run = TestRun::new("diagnostic_name", "my_dut", "1.0");
    /// test_run.scope(|r| {
    ///     r.log(LogSeverity::Info, "First message")?;
    ///     TestRunOutcome {
    ...
    /// let test_run = TestRun::new("diagnostic_name", "my_dut", "1.0");
    /// test_run.start().await?;
    /// test_run.error("symptom", "error messasge")?;
    /// test_run.end(TestStatus::Complete, TestResult::Pass)?;
    /// ```
...
impl TestRunBuilder {
    fn new(name: &str, dut: &objects::DutInfo, version: &str) -> Self {
        Self {
            name: name.to_string(),
            dut: dut.clone(),
...
impl TestStep {
    ...
    /// step.scope(|s: &TestStep| -> Result<TestStatus, WriterError> {
    ///     s.log(
    ///         LogSeverity::Info,
    ///         "This is a log message with INFO severity",
    ///     )?;
    ...
    /// let step = test_run.step("step_name")?;
    /// step.start()?;
    /// ocptv_log_info!(step, "This is a log message with INFO severity")?;
    /// step.end(TestStatus::Complete)?;
    /// ```
    pub async fn log(
        &self,
    ...
    /// let step = test_run.step("step_name")?;
    /// step.start()?;
    /// step.error("symptom", "error message")?;
    /// step.end(TestStatus::Complete)?;
    /// ```
    ///
    /// ## Using macros
    ...
    /// let step = test_run.step("step_name")?;
    /// step.start()?;
    /// ocptv_error!(step, "symptom", "error message")?;
    /// step.end(TestStatus::Complete)?;
    /// ```
    pub async fn error_with_msg(
        &self,
    ...
    /// let step = test_run.step("step_name")?;
    /// step.start()?;
    /// step.add_measurement("name", Value::from(50))?;
    /// step.end(TestStatus::Complete)?;
    /// ```
    ...
    ///     .hardware_info_id("id")
    ///     .add_validator(&Validator::builder(models::ValidatorType::Equal, Value::from(30)).build())
    ///     .add_metadata("key", Value::from("value"))
    ///     .subcomponent(&Subcomponent::builder("name").build())
    ///     .build();
    /// step.add_measurement_with_details(&measurement)?;
    /// step.end(TestStatus::Complete)?;
    /// ```
    ...
    /// let step = test_run.step("step_name")?;
    /// step.start()?;
    /// let series = step.measurement_series("series_id", "name");
    /// ```
...
impl MeasurementSeries {
    ...
    /// let series = step.measurement_series("series_id", "name");
    /// series.start()?;
    /// series.add_measurement(Value::from(60))?;
    /// ```
    ...
    /// let series = step.measurement_series("series_id", "name");
    /// series.start()?;
    /// series.add_measurement_with_metadata(Value::from(60), vec![("key", Value::from("value"))])?;
    /// ```
    ...
    /// series.start()?;
    /// series.scope(|s| {
    ///     s.add_measurement(Value::from(60))?;
    ///     s.add_measurement(Value::from(70))?;
    ///     s.add_measurement(Value::from(80))?;
    ///     Ok(())
    /// })?;
    /// ```
    pub async fn scope<'a, F, R>(&'a self, func: F) -> Result<(), emitters::WriterError>
...
mod tests {
    ...
    #[tokio::test]
    async fn test_testrun_with_log() {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
            json!({"sequenceNumber":2,"testRunArtifact":{"testRunStart":{"dutInfo":{"dutInfoId":"dut_id","hardwareInfos":null,"metadata":null,"name":null,"platformInfos":null,"softwareInfos":null},"metadata":null,"name":"run_name","parameters":{},"version":"1.0"}}}),
    ...
    #[tokio::test]
    async fn test_testrun_with_log_with_details() {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
            json!({"sequenceNumber":2,"testRunArtifact":{"testRunStart":{"dutInfo":{"dutInfoId":"dut_id","hardwareInfos":null,"metadata":null,"name":null,"platformInfos":null,"softwareInfos":null},"metadata":null,"name":"run_name","parameters":{},"version":"1.0"}}}),
    ...
    #[tokio::test]
    async fn test_testrun_with_error() {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
            json!({"sequenceNumber":2,"testRunArtifact":{"testRunStart":{"dutInfo":{"dutInfoId":"dut_id","hardwareInfos":null,"metadata":null,"name":null,"platformInfos":null,"softwareInfos":null},"metadata":null,"name":"run_name","parameters":{},"version":"1.0"}}}),
        ...
        let test_run = TestRun::builder("run_name", &dut, "1.0")
            ...
            .build();
        test_run.start().await.unwrap();
        test_run.error("symptom").await.unwrap();
        test_run
            .end(TestStatus::Complete, TestResult::Pass)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_testrun_with_error_with_message() {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
            json!({"sequenceNumber":2,"testRunArtifact":{"testRunStart":{"dutInfo":{"dutInfoId":"dut_id","hardwareInfos":null,"metadata":null,"name":null,"platformInfos":null,"softwareInfos":null},"metadata":null,"name":"run_name","parameters":{},"version":"1.0"}}}),
        ...
        test_run.start().await.unwrap();
        test_run
            .error_with_msg("symptom", "Error message")
            .await
            .unwrap();
        test_run
            .end(TestStatus::Complete, TestResult::Pass)
    ...
    #[tokio::test]
    async fn test_testrun_with_scope() {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
            json!({"sequenceNumber":2,"testRunArtifact":{"testRunStart":{"dutInfo":{"dutInfoId":"dut_id","hardwareInfos":null,"metadata":null,"name":null,"platformInfos":null,"softwareInfos":null},"metadata":null,"name":"run_name","parameters":{},"version":"1.0"}}}),
    ...
    async fn test_step_with_measurement() -> Result<()> {
        ...
        let step = test_run.step("first step")?;
        step.start().await?;
        step.add_measurement("name", Value::from(50)).await?;
        step.end(TestStatus::Complete).await?;
    ...
    async fn test_step_with_measurement_builder() -> Result<()> {
        ...
        let measurement = Measurement::builder("name", Value::from(50))
            ...
            .add_validator(
                &objects::Validator::builder(models::ValidatorType::Equal, Value::from(30)).build(),
            )
            .add_metadata("key", Value::from("value"))
    ...
    async fn test_step_with_measurement_series_element() -> Result<()> {
        ...
        let series = step.measurement_series("name");
        series.start().await?;
        series.add_measurement(Value::from(60)).await?;
        series.end().await?;
        step.end(TestStatus::Complete).await?;
    ...
    async fn test_step_with_measurement_series_element_index_no() -> Result<()> {
        ...
        series.add_measurement(Value::from(60)).await?;
        series.add_measurement(Value::from(70)).await?;
        series.add_measurement(Value::from(80)).await?;
        series.end().await?;
        step.end(TestStatus::Complete).await?;
    ...
    async fn test_step_with_measurement_series_element_with_metadata() -> Result<()> {
        ...
        series.start().await?;
        series
            .add_measurement_with_metadata(Value::from(60), vec![("key", Value::from("value"))])
            .await?;
        series.end().await?;
        step.end(TestStatus::Complete).await?;
    ...
    async fn test_step_with_measurement_series_element_with_metadata_index_no() -> Result<()> {
        ...
        series
            ...
            .await?;
        series
            .add_measurement_with_metadata(Value::from(70), vec![("key2", Value::from("value2"))])
            .await?;
        series
            .add_measurement_with_metadata(Value::from(80), vec![("key3", Value::from("value3"))])
            .await?;
        series.end().await?;
        step.end(TestStatus::Complete).await?;
    ...
    async fn test_step_with_measurement_series_scope() -> Result<()> {
        ...
        series
            .scope(|s| async {
                s.add_measurement(Value::from(60)).await?;
                s.add_measurement(Value::from(70)).await?;
                s.add_measurement(Value::from(80)).await?;
                Ok(())
            })
            .await?;
        step.end(TestStatus::Complete).await?;
    ...
    #[tokio::test]
    async fn test_config_builder() -> anyhow::Result<()> {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
            json!({"sequenceNumber":2,"testRunArtifact":{"testRunStart":{"dutInfo":{"dutInfoId":"dut_id","hardwareInfos":null,"metadata":null,"name":null,"platformInfos":null,"softwareInfos":null},"metadata":null,"name":"run_name","parameters":{},"version":"1.0"}}}),
            json!({"sequenceNumber":3,"testRunArtifact":{"error":{"message":"Error message","softwareInfoIds":null,"sourceLocation":null,"symptom":"symptom"}}}),
    ...
    #[tokio::test]
    async fn test_testrun_instantiation_with_new() {
    ...
    #[tokio::test]
    async fn test_testrun_metadata() -> anyhow::Result<()> {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
    ...
    #[tokio::test]
    async fn test_testrun_builder() -> anyhow::Result<()> {
        let expected = [
            json!({"schemaVersion":{"major":2,"minor":0},"sequenceNumber":1}),
        ...
        let test_run = TestRun::builder("run_name", &dut, "1.0")
            ...
            .add_parameter("key", Value::from("value"))
